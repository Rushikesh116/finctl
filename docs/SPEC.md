...
| `payment_id` | `str \| None` | set on `refund` and `transfer` |
| `order_id` | `str \| None` | |
...
| 5 | Chargeback followed by a representment reversal | Signed adjustments | **No `chargeback` type exists** — generate as a row with `dispute_id` populated, then the reversal leg |
